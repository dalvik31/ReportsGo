...
import com.epacheco.reports.compose_reformat.ui.home.bottom_screens.orders.main_orders_detail.OrdersUiIntent
...
class NewOrderViewModel @Inject constructor(
    ...
    fun handleIntent(intent: NewOrderUiIntent) {
        when (intent) {
            is NewOrderUiIntent.CreateOrder -> createOrder(intent.mainOrderId, intent.orderSeason)
            is NewOrderUiIntent.DeleteOrder -> deleteOrder(intent.orderId, intent.mainOrderId)
            NewOrderUiIntent.HideDialogs -> setErrorMsg()
            is NewOrderUiIntent.UpdateOrder -> updateOrder(intent.order)
            ...
            when (val orderMainMainResponse = createOrderUseCase(
                ...
                is Resource.Success -> {
                    if (validInputs()) {
                        ...
                        _uiState.value =
                            _uiState.value.copy(errorMessage = app.getString(R.string.order_empty_inputs_error))
